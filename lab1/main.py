...
for num_nodes in [10, 15, 20]:
    ...
    for i in range(1, num_nodes - 1):
        ...
        b_nodes[i] = 6 * ((y_nodes[i + 1] - y_nodes[i]) / h_nodes[i] -
                          (y_nodes[i] - y_nodes[i - 1]) / h_nodes[i - 1])
    ...
    yy_interp = np.array([spline_eval_nodes(xi) for xi in x])
    errors_dict[num_nodes] = np.abs(y - yy_interp)   # <-- ПО МОДУЛЮ

# -------------------------------------------------
# 11. Спільний графік похибки (по модулю)
# -------------------------------------------------
plt.figure()
for num_nodes in [10, 15, 20]:
    plt.plot(x, errors_dict[num_nodes], label=f'{num_nodes} вузлів')

plt.title("Похибка апроксимації (по модулю)")
plt.xlabel("Distance (m)")
